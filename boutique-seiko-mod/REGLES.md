...
**Retirer un média se fait par détachement, jamais par suppression** (incident du 12/08/2026). Le retrait d'un média produit passe **toujours** par `fileUpdate` + `referencesToRemove` : le fichier reste dans les fichiers Shopify et l'opération est réversible. **`fileDelete` est interdit sur un média produit.** Le 12/08, 61 visuels maison ont été retirés à tort de 37 fiches actives ; 37 d'entre eux avaient été passés à `fileDelete` et n'existaient plus — ils n'ont été récupérés que parce qu'une copie traînait dans `livraisons/`.

**Une classification « fournisseur par défaut » ne déclenche jamais un retrait** (même incident). L'audit du 12/08 étiquetait « fournisseur/AliExpress » *tout média dont le fichier local n'était pas retrouvé*, puis supprimait sur cette base. Un visuel maison dont on a perdu la trace locale reste un visuel maison. **L'absence de preuve n'est pas une preuve d'origine fournisseur** : dans le doute, on ouvre l'image et on regarde.
