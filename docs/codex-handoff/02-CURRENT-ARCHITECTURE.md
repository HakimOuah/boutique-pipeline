...
|---|---|---|
| **Shopify Admin GraphQL** (connecteur MCP) | Produits, collections, publications (`publishablePublish`), metafields, staged uploads, `themeFilesUpsert` sur thème brouillon — tout le build Noirmont | Refuse d'écrire sur un thème MAIN ; **`switch-shop` interdit** (invalide la connexion pour tous) ; requêtes média plafonnées à 30 ; écritures thème asynchrones (`upsertedThemeFiles: []` ≠ échec) **[FAIT — repo:REPRISE-SESSION.md §Pièges]** |
...
| **Higgsfield** (images IA) | Visuels produit Noirmont (galeries, swatches) | Imprime de faux logos sur les cadrans → compositions cachant le cadran + inpainting OpenCV local **[MÉMOIRE — shopify-canal-et-visuels-ia.md]** + `boutique-seiko-mod/visuels-2026-07-25/` **[FAIT — repo]** |
...
| SEMrush (compte **payant**, récent) | Volumes FR, Keyword Magic, mesure express | ⚠️ [CONTRADICTOIRE — `../CONTEXTE-MEMOIRE-pour-Codex.md` (23/06 : « pas de compte permanent, essais ponctuels, Semrush OFF par défaut ») vs `boutique-seiko-mod/REPRISE-SESSION.md` + `marche-complet-semrush.md` (27/07 : « SEMrush (compte payant) »)]. Résolution probable : abonnement souscrit entre-temps ; la mention PLAYBOOK « Semrush désactivé par défaut » est périmée. **À valider par Hakim.** |
