# 18/08/2026 — USP du gabarit produit (argumentaire site)

Hakim : remplacer les paragraphes des 4 blocs image/texte du template produit par des
USP alignés sur l'argumentaire marketing.

## Constat préalable

`copie-de-fullstack-2-3` (`205568147794`) est **MAIN** (plus UNPUBLISHED). Écriture
sur le live, `--allow-live`, fichier seul `templates/product.json`. Titres / kickers
inchangés. Backup : `backups/2026-08-18-usp-pdp/product-avant.json`.

## Remplacements (paragraphes seuls)

| Bloc | Avant | Après |
|---|---|---|
| Notre sélection | « équipements fiables et rechargeables… pièce par pièce » | Verre sans goût de chlore, douche sans traces, plus de packs — on commence par ce qui agace |
| En quelques minutes | Inventaire pose (filtre / carafe / osmoseur) | Vous le posez vous-même. Pas de devis. Si ça demande plus, c'est sur la fiche |
| Transparence | Specs vs non vérifié (ton conformité) | On dit ce qui est prouvé et ce qui ne l'est pas |
| Du filtre à l'osmoseur | Catalogue qui « couvre tout le parcours » | Le bon outil, pas le plus cher. Coût d'entretien avant le panier |

Source : persona Claire + axe « transparence radicale » + hero home (packs, chlore,
point d'usage). 0 claim santé, 0 taux inventé. Formules **pas** copiées mot pour mot
du hero.

## Constaté

`https://bonumvitae.fr/products/kit-entretien-osmoseur-600-gpd` — les 4 nouveaux
paragraphes sont dans le HTML. Anciens textes absents.
