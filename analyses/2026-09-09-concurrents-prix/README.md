...
Correction liens (09/09, retour Hakim) : 25 fiches retenues sur 124 n'ont pas d'identifiant produit Google (offres « headline » sans page produit) et pointaient vers une URL invalide. Elles sont désormais résolues par recherche organique DataForSEO (`organic-links.json`, page du marchand dont le titre recoupe le mieux l'offre), sinon par la page d'offre Google Shopping (`shopping_url`). Les liens résolus par `sellers` (99 fiches) répondent 200 ou 403 en curl (403 = anti-robot Leroy Merlin/ManoMano/OOGarden, pages normales dans un navigateur).
